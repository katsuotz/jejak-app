...
<div class="flex flex-col gap-4 px-4 py-6 max-w-lg mx-auto">
	<div>
		...
		<p class="text-sm text-muted-foreground mt-1">Built-in workout programs with auto BPM changes</p>
	</div>
			...
			>
				<div class="flex items-center justify-between">
					...
					<div class="flex items-center gap-2">
						<span class="text-xs font-medium {difficultyColor(template.id)}">{difficultyLabel(template.id)}</span>
						<span class="rounded-md bg-secondary px-2 py-0.5 text-xs font-medium text-muted-foreground">{template.totalDurationMinutes} min</span>
					</div>
				</div>
				...
				<div class="flex w-full gap-0.5 rounded-full overflow-hidden h-2 mt-1">
					{#each template.phases as phase}
						...
						<div
							class="h-full rounded-sm {phase.bpm >= 180 ? 'bg-red-500' : phase.bpm >= 170 ? 'bg-yellow-500' : 'bg-green-500'}"
				...
				<div class="flex flex-wrap gap-x-3 gap-y-1 mt-1">
					{#each template.phases as phase, i}
						...
						{@const isDuplicate = i > 0 && phase.name === template.phases[i - 1].name && phase.bpm === template.phases[i - 1].bpm}
						{#if !isDuplicate}
							<span class="text-[10px] text-muted-foreground">
								{phase.name.replace(/ \(\d+\/\d+\)/, '')} {phase.bpm}
							</span>
						{/if}
					{/each}
				</div>
